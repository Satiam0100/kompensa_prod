    ...
    <div className="space-y-4 animate-pulse">
      ...
      <div className="hidden lg:block bg-surface-container border border-outline-variant rounded-lg overflow-hidden">
        ...
        {Array.from({ length: 6 }).map((_, i) => (
          <div
            key={i}
            className="h-14 border-b border-outline-variant/40 px-4 flex items-center gap-4"
          >
      ...
      <div className="lg:hidden grid gap-3">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="h-24 bg-surface-container border border-outline-variant rounded-lg"
          />
        ))}
      </div>
    </div>
